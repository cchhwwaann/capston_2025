...
DEMO_SEQUENCE = [
    ...
    {'name': "2. 붐 올리기 (2초)", 'cmd': 'k', 'stop': 'K', 'duration': 2.0},
    {'name': "3. 암 오므리기 (2초)", 'cmd': 'w', 'stop': 'W', 'duration': 2.0},
    {'name': "4. 붐 내리기 (2초)", 'cmd': 'i', 'stop': 'I', 'duration': 2.0},
    {'name': "5. 대기 (0.5초)",    'cmd': None, 'stop': None, 'duration': 0.5},
]
...
# CSV 헤더 (S1=Bottom, S2=Top)
writer.writerow(['Time', 'Boom_Bottom_Rel(S1)', 'Boom_Top_Rel(S2)', 'Bucket_X', 'Bucket_Y', 'Bucket_Z']) 
...
# 🎨 [수정] Sensor 2 -> Boom Top (파란색)
ax1.set_title("Boom Top (Sensor 2)")
ax1.set_ylabel("Relative")
ax1.grid(True)
line1, = ax1.plot([], [], 'b-', label='Boom Top') # Blue

# 🎨 [수정] Sensor 1 -> Boom Bottom (빨간색)
ax2.set_title("Boom Bottom (Sensor 1)")
            ...
            if frame is not None:
                ...
                if demo_running:
                    if demo_step_idx < len(DEMO_SEQUENCE):
                        status = f"MODE: DEMO ({DEMO_SEQUENCE[demo_step_idx]['name']})"
                        col = (0, 0, 255)
                    else:
                        status = "MODE: DEMO (Finishing...)"
            ...
            if demo_running:
                if not is_calibrated: pass 
                    ...
                    if demo_step_idx < len(DEMO_SEQUENCE):
                        ...
                        print("✅ 시연 종료. 수동 모드 복귀.")
                        demo_running = False
                        send_cmd('I'); time.sleep(0.1)
                        send_cmd('W'); time.sleep(0.1)
                        send_cmd('J')
                ...
                while ser.in_waiting > 0:
                    ...
                    if ',' in line:
                        ...
                        if len(parts) >= 2:
                            ...
                            # 🔄 [매핑 변경] S1(Bottom) -> Data 1, S2(Top) -> Data 2
                            data1.append(rel1) 
                            data2.append(rel2)
            except: pass
